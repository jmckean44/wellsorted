...
function initTrig() {
	if (!trig) {
		...
		window.removeEventListener('scroll', trig.trigScroll, { passive: true });
		window.removeEventListener('resize', trig.trigWindowHeight, { passive: true });
	}
...
document.addEventListener('DOMContentLoaded', initTrig);
